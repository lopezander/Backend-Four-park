...
module.exports = {
    ...
    async create(usuario) {
        try {
            const { correo_electronico, nombre, apellido } = usuario;
            const contrasena = v4();
            ...
            const result = await pool.query(
                ...
                [correo_electronico, '2' , nombre, apellido, hash]
            );
    ...
    async findAll() {
        const result = await pool.query("SELECT * FROM usuario where id_rol =$1",['2']);
        return result.rows;
    },

    async findById(correo) {
    ...
    async update(id, usuario) {
        ...
        const result = await pool.query(
            ...
            [id, '2', nombre, apellido, contrasena]
        );
        return result.rows[0];
    },
    async delete(correo) {
